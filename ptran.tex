...
% $Id: ptran.tex,v 1.21 2002-07-24 18:31:41 cananian Exp $
...
A variable $v$ is conditionally-singular on $P$ at $n$ if
$P=\text{CondParam}(v,n)$ and $\neg\text{Ns}(v,n)$.  This
is identical to our former case of non-conditional singularity
when $P=\emptyset$.  Statements $s_1$ and $s_2$ are conditionally
singular on $P$ if
\begin{eqnarray*}
\neg\exists v_1\in\text{use}(s_1), v_2\in\text{use}(s_2),
     d &:& \tuple{v_1, d} \in \text{In}_\text{Gs}(s_1) \wedge
...
Similarly, a set of statements $S$ is mutually-conditionally-singular on $P$
if every pair in $S$ is pairwise-singular and
\begin{displaymath}
P = \bigcup_{s\in S} \bigcup_{v\in\text{use}(s)} \text{CondParam}(v,s)
\end{displaymath}

Now we can resolve conditional singularity at a callsite by
examining actual parameters.
...
% this isn't completely correct, but it's the right idea.
A set $S_1 \cup S_2$ of statements, where
 $S_1 \subseteq M_1$ the set of methods in the caller, and 
 $S_2 \subseteq M_2$ the set of methods in the callee,
is mutually singular iff
 $S_2$ is mutually singular conditional on set of parameters $P$,
 $S_1$ is mutually singular,
 and no path from $s_1 \in S_1$ to callsite or
             from callsite to $s_1\in S_1$,%
\footnote{Assuming METHODENTER in $M_2$ reaches all $s_2\in S_2$ and
          all $s_2\in S_2$ reach METHODEXIT in $M_2$.}
