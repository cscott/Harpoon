...
% $Id: scool05.tex,v 1.2 2005-08-05 21:00:11 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{abstract}
  ...
  performance for long-running transactions.  A model for the software
...
for the concerns of this paper is fault-tolerance: processes which
...
\begin{itemize}
\item Empirical data on transaction properties from ``transactified''
  Java programs.
%\item A minimal syntax to express transactions in a high-level manner
%  to provide fault-tolerance, backtracking, and non-blocking 
%  synchronization.
%\note{This is \texttt{try \{\ldots\} else \{\ldots\}} syntax.}
\item A design for an ``object-oriented'' implementation of transactions.
\item An efficient implementation of this design.
\item A mechanically-checked Promela model of the implementation.
\punt{
...
Schorr and Waite \cite{SchorrWa67}.

In normal execution, the data structure is left complete and intact
after the operation.  But
imagine that an exception or fault occurs
at some point during the traversal: control may
leave the traversal, but the data structure is left in
...
vertex and decreasing it by the same amount on another vertex.
...
data-cache read/write ratios others have collected \cite[p105,379]{Hennessy96}.
...
Although space does not permit a full discussion; it is desirable that
an STM system cooperate well with some HTM system, in order to obtain
good performance for frequent small transactions (Quads I and II).
...
We use the term \emph{non-blocking} to describe
...
protocols of LaMarca and Alemany and Felten~\cite{AlemanyFe92}, and a
