...
% $Id: scool05.tex,v 1.10 2005-08-06 04:40:03 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\verbatim}%
  {\endverbatim}
% tighten up captions
\renewcommand{\figadjust}{\vspace{-1em}}
...
object-oriented transaction system. We will present a mechanically-checked
non-blocking object-oriented implementation, and experimental
measurements of transactional code justifying its design.  We will
also provide one solution to the ``large object'' problem.

\secput{why}{Using Objects for Performance}
Recent trends in lightweight transaction system design have used
a \emph{transactional memory} abstraction.  The success of Hardware
Transactional Memory (HTMs)
\cite{Knight86,HerlihyMo92,HerlihyMo93,HammondWoCh04,AnanianAsKuLeLi04},
with word- or cache-line--based abstractions to match the standard
cache/memory system,
have inspired similar Software Transactional Memory (STM) systems
\cite{ShavitTo95,HarrisFr03}.  We suggest that the
exclusive\footnote{See \secref{hybrid}.}
...
transactions which they perform.
...
Any scheme which allows the programmer free choice of
...
although for these they are very efficient.  Object-oriented
...
An ideal system would combine an HTM and an object-oriented
...
object-oriented software transaction system.
...
strengths of our object-oriented software transaction system with a
...
object-oriented sofware system.  The field flag mechanism described in
...
the \texttt{ReadNT} and \texttt{WriteNT} algorithms for a slight overhead.
