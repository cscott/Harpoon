...
% $Id: scool05.tex,v 1.25 2005-10-07 14:38:54 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\category{D.3.2}{Pro\-gram\-ming Languages}{Language
  Classifications}[Object-oriented languages, Java]
\category{D.3.3}{Program\-ming Languages}{Language Constructs and
...
although for these they are very efficient.  However, large and
long-lived transactions can be readily seen in 
\figref{tr-sz-all}, which plots
the distribution of transaction sizes in SPECjvm98
on a semi-log scale.  Object-based transaction systems pay copying
overhead up front, making them costly for small transactions, but have
low subsequent per-operation costs, which make them the best choice
...
This read barrier is the only overhead required for non-transactional
code.  As the test is simple and predictable, the runtime cost can be
very low: the compare-and-branch can be scheduled in otherwise unused
instruction slots on a multiple-issue processor \cite{HudsonMoSuWa00}.
...
for the SPECjvm98 benchmark applications.  Note that in some cases
bloat actually increases performance, due to fortuitous alignment and
cache effects.}
...
mean of 2\% (and no more than 10\%) slowdown on these benchmarks,
including costs caused by increased heap size and more-frequent
garbage collection.
This is consistent with the results reported by Bacon, Fink, and
Grove, who found that reducing object header size from two words to one
gave speedups ranging from -1.5\% to 2.2\% \cite{BaconFiGr02}.
...
instruction.  Load-Linked/Store-Conditional also
provides a convenient solution to the so-called ``ABA'' problem of
Compare-And-Swap.\footnote{Store-Conditional will fail if the target
  of the Load-Linked is written to, even if the value written is
  identical to value previously in the location.}
...
\texttt{0xCA} to be our flag value, repeated as
necessary\footnote{Slight scatological pun intended.} to fill out
...
\FLAG, the thread must copy back the field value 
...
field really is \FLAG.  The {\tt kill\_writers} constant
is passed to the copy-back procedure to
...
to field $f$ of object $o$.  For correctness, the thread needs to ensure that
the reader list is empty before it does the write.  We implement this
with a Load-Linked/Store-Conditional pair (modelled slightly
differently in Promela), ensuring that the write only takes effect
...
If it is not empty, the thread must
...
\end{figure}
\figref{copyback} presents the field copy-back routine.  The thread creates a
pseudo-version owned by a pre-aborted transaction which serves as a
reservation on the head of the version list.  It then writes to the
object field (with a Load-Linked/Store-Conditional pair) if and only if
its pseudo-version remains at the head of the versions list.\footnote{Note
  again that a CAS does not suffice.}  We want to avoid being
interrupted while writing back some value $A$, and having a concurrent
transaction finish writing back $A$ and then commit and write back
some other value $B$, before we resume and clobber $B$ with out
(interrupted) write-back of $A$.  The use of the LL/SC on the head of
the version list prevents this possible race.

\subsubsection{Transactional Read}
A transactional read is split into two parts.  Before the read, the thread
must ensure that its transaction is on the reader list for the
object.  This is straight-forward to do in a non-blocking manner as
long as all inserts are to the head of the list.  The thread must also
walk the versions list, and abort any uncommitted transaction other
than its own.  With sufficiently precise pointer analysis these steps
can be combined and hoisted so that they are done once before the
first read from an object and not repeated.  Imprecision in the
analysis may cause re-examination of the reader list.

At read time, the thread initially reads from the original object.  If
the value read is not \FLAG, the read value is used.  Otherwise, the
thread uses {\tt findVersion} to look up the version object {\tt ver}
associated with its transaction 
(this will typically be at the head of the version list) and then reads the
appropriate value from that version.  Note that the initial
read-and-check can be omitted if it knows that it has already written
to this field inside this transaction, and that once {\tt ver} has
been looked up, it can be used in subsequent calls to {\tt readT} (it
is both an input and an output argument to the function).  If there is
not an uncommitted version for the object, {\tt findVersion} returns
the committed version in {\tt \_r}.
\begin{inlinecode}
...
Like the transactional read, transactional writes are split.  The
first preparatory step is done once for each object written by the
transaction.  The version list for each object is traversed, aborting
other versions and locating or creating a version {\tt ver} for the
object corresponding to the current transaction.  We retain {\tt ver}
for the actual write, later.  The reader list must also be traversed,
aborting all transactions on the list except the current transaction.
This is shown in the {\tt ensureWriter} routine in
\figref{ensurewriter}.

The second preparatory step is done once for each field we intend to write.
The thread must perform a copy-through on each field:
copy the object's (pre-write) field value into all the versions and
then write \FLAG to the object's field.  We use
Load-Linked/Store-Conditional to update versions only if the object's
field has not already been set to \FLAG behind our backs by a concurrent
copy-through.  The {\tt checkWriteField} routine is shown in
\figref{copythrough}.

Finally, for each transactional write to an object, we simply write to the
identified version {\tt ver} for that object.
\begin{inlinecode}
...
\end{inlinecode}
Note that we have chosen not to allow concurrent writes by separate
transactions to different fields in the same object.
...
Our solution is to represent large objects as \defn{functional
  arrays}.  Functional arrays allow access to both committed and
uncommitted versions of the (large) object \emph{without} requiring
multiple complete copies.  O'Neill and Burton \cite{ONeillBu97} give a fairly
inclusive overview of functional array data structures; we will review very
briefly here.

Functional arrays are \emph{persistent}; that is, after an element is
updated both the new and the old contents of the array are available
for use.  Since arrays are simply maps from integers (indexes) to
values; any functional map datatype (for example, a functional
balanced tree) can be used to implement functional arrays.
...
\end{itemize}
We are interested in lock-free implementations, thus we
allow any of these operations to \emph{fail}.  Failed operations
...
\begin{itemize}
\item $\funcname{FA-Create}_{so}(n)$: Return an ordinary imperative array of size
  $n$.
\item $\funcname{FA-Update}_{so}(A_j, i, v)$: Create a new imperative array
  $A_{j'}$ and atomically copy the contents of $A_j$ to $A_{j'}$.
  Return $A_{j'}$.
\item $\funcname{FA-Read}_{so}(A_j, i)$: Return $A_j[i]$.
\end{itemize}
This implementation has $O(1)$ read and $O(n)$ update, so it matches
the performance of imperative arrays only when $n=O(1)$, where $n$ is
the size of the array (read, object).  We will therefore call these
\emph{small object functional arrays}.  We can view the version
objects we have so far described as implementations of such an
algorithm, where $\funcname{FA-Update}_{so}$ takes an object and
returns a version (slightly abusing the types).  This is implemented
within {\tt ensureWriter}, \figref{ensurewriter}.

To handle large objects, we replace this na{\"\i}ve implementation in
order to scale better with large $n$.  We would like to prevent small
changes to a large object from taking a large amount of time.  We
could consider using a functional array implementation based on
functional balanced trees.  However, this yields $O(\lg n)$ access or
update, which is not as good as the $O(1)$ element access and update
characteristic of imperative arrays.  Instead, we've chosen Tyng-Ruey
Chuang's version \cite{Chuang94} of \emph{shallow binding}, which
gives $O(1)$ cost for \funcname{FA-Update} and for single-threaded
\funcname{FA-Read}.  Our use of functional arrays is single-threaded
in the common case, when transactions do not abort.  Chuang's scheme
uses randomized cuts to the version tree to limit the expected
per-write overhead to $O(n)$ in the worst case (that is, the expected
cost $C(l,m)$ of $l$ updates and $m$ reads obeys $C(l,m) \leq l + m +
2nl$).  So with little complexity we can get worst case expected
performance for aborts equivalent to the na{\"\i}ve
implementation, while obtaining ``imperative-speed'' common-case
(commit) accesses.

Of course, Chuang's published algorithm is not lock free.
...
can trivially make Chuang's published algorithm lock-free by
implementing the rotation using some existing small-transaction HTM.
...
\caption{Performance (in cycles per node push on a simple queue
  benchmark) of a simple bounded hardware transactional memory (HTM), the
...
fast bounded-size HTM.  The numbers presented are cycle counts from a
cycle accurate multiprocessor simulator (UVSIM~\cite{Zhang03}),
extended to implement a HTM for \cite{AnanianAsKuLeLi04}.  More
details are available in \cite{Lie04}.

In the figure, combining the systems is done
in the most simple-minded way: all transactions are begun in
the hardware transactional memory,
...
interact properly with concurrent software transactions, although this
behavior can be done in software (it does not need to be part of the
...
transactions.  No special effort was made to tune code or minimize
this, and the processor simulated had limited ability to exploit ILP
(2 ALUs and 4-instruction issue width).  However, the read barriers
might be a worthwhile target for hardware support \cite{ClickTeWo05}.

Hardware support for small transactions may also be used to implement
the software transaction implementation's
Load-Linked/Store-Conditional atomic sequences, which may not
otherwise be available on a target processor.

\secput{verification}{Verification}
The Promela model of this software transaction system was
model-checked with Spin version 4.1.0 and verified to operate
correctly and without races in the scenarios described below.  The
verification was done on an SGI 64-processor MIPS machine with 16G of
memory.
...
Different test scripts were run separately against the
model, using separate processors of the verification machine (Spin can
not otherwise exploit SMP).  Some representative costs include:
\begin{itemize}
...
\item Testing a sequences of transactional and non-transactional reads
...
In the process one bug in Spin 4.0.7 was discovered and corrected (in
Spin 4.1.0) and a number of subtle race conditions were discovered and
...
subtle-but-legitimate race conditions, including:
\begin{itemize}
\item A race allowing conflicting readers to be created while a writer
  was creating a new version object in {\tt ensureWriter}.
\item Allowing already-committed version objects to be mutated when {\tt
  ...
  a number of subtle race conditions in {\tt ensureWriter} and {\tt
    copyBackField}.  The code that was expected to manage these races
  had unexpected corner cases.
\item Using a {\tt readers} bitmask to provide per-field granularity
  to the {\tt readerList} proved unmanageable as a large number of
  unforeseen races were discovered.
\end{itemize}
In addition, the model proved a valuable design tool, as portions of
the algorithm could readily be mechanically checked to validate (or
discredit) the designer's reasoning about the concurrent system.
Humans do not excel at exhaustive state space exploration.
...
The design has been model-checked with the Spin model checker, and
...
\figref{hybrid}.  Some additional details are in \cite{Lie04}.
