...
% $Id: thesis.tex,v 1.65 1999-08-18 16:05:26 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\pagestyle{myheadings}\markboth{$ $Revision: 1.65 $ $}{$ $Revision: 1.65 $ $}
\pagenumbering{roman}
...
\input{Figures/THsccalg1.tex}
\caption{SCC algorithm for SSA form.}\label{alg:scc}
\end{myalgorithm}
\begin{myalgorithm}\small
\input{Figures/THsccalg2.tex}
...
table~\ref{tab:sccmeet1}. The time complexity of SCC is
$O(N+U_{SSA})$, where $U_{SSA}$ is the number of variable
\newterm{uses} in the SSA representation of the program.  This can be
verified easily: the procedure \code{RaiseE} puts each node on the
$W_n$ worklist at most once, and \code{RaiseV} puts a variable on the
$W_v$ worklist at most $D$ times, where $D$ is the maximum lattice
depth.  The \code{Visit} procedure can thus be invoked a maximum of
$N$ times by line~\ref{line:visitWn} of algorithm~\ref{alg:scc}, and a
maximum of $U_{SSA}D$ times by line~\ref{line:visitWv}.  The
lattice depth $D$ is the constant 2 in this version of the algorithm,
yielding the previously cited time complexity.
...
SCC/SSA: $O(N+U_{SSA})$, by the same argument as before.
...
\cite{javabytecode}; the extended lattice is presented in figure
\ref{fig:scclat2}.
...
identical to that of algorithm \ref{alg:sccssi}.
...
as the class hierarchy, and the roots and leaves are fully linked to
the lattice elements above and below it.  Because the lattice is
deeper, the asymptotic runtime complexity is now $O(E+U_{SSA}D)$
where $D$ is the maximum depth of the class hierarchy.  
To form an estimate of the magnitude of $D$, table \ref{tab:chstats}
...
infrastructure is under continuing development.} one can infer that
the $U_{SSA}D$ term in the runtime complexity is not likely to make
the algorithm non-linear.
...
we will also use as roots.  The primitive types include what we will
refer to as the \code{void} type, which is the type of the expression
\code{null}, as well as \code{int}, \code{long}, \code{float}, and
...
\code{int}.}  The ability to represent the \code{void} type begins to
allow us to address null-pointer checks, although because
\code{meet(null, x)==x}\fixme{write up meet and join rules.} we can
only reason about variables which can be proven to be null, not those
which might be proven to be non-null (which is the more useful case).
The next section will provide a more satisfactory treatment.
...
and added a lattice level for arrays of known constant length.  The
new meet and join rules are described in figure \ref{fig:nothereyet}.
...
\fixme{Note that SSI form is \textbf{essential} in allowing SPTC to do
array-bounds and null-pointer checks in a useful manner.}
