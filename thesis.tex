...
% $Id: thesis.tex,v 1.17 1999-06-10 15:39:35 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{document}
\pagestyle{myheadings}\markboth{$ $Revision: 1.17 $ $}{$ $Revision: 1.17 $ $}
...
\begin{myfigure}\label{fig:tossa}
\begin{center}
...
\begin{myfigure}\label{fig:prunedssa}
\begin{center}
...
\begin{myfigure}\label{fig:tossi}
\begin{center}
...
our example program.
\begin{myfigure}\label{fig:prunedssi}
\begin{center}
...
The construction of SSI form builds upon the linear-time
cycle-equivalancy algorithm described in \cite{johnson93:sese}.
However, since the published algorithm is incorrect in a number of
points, we will re-present it briefly here.
...
Procedure cycle_equiv (G: CFG)
{
  ...
  Expand each node and undirect edges as described in section FOO;
  Perform depth-first search of undirected graph as described in
    section BAR, assigning preorder number to each node (low numbers
    are closest to START, hence 'highest');
...
of type $V^* \to V$.  If any operator argument is $\bot$, the
...
%	 \hfill \\ \: % line-break
		...
		\hfill \\ & \quad % line-break % FIXME: remove space.
...
$\bot_V$.
\item A \emph{configuration} consists of an environment. 
The initial configuration is $\rho_\emptyset$, that is,  all names in $N$ are
mapped to $\bot_V$. \textbf{FIXME: START CONDITION.}
\end{enumerate}
\end{definition}
...
\emph{event set} and an \emph{invariant store}.
The event set $E$ contains definitions of the form $t=c$, and the invariant
...
\item A \emph{configuration} is a tuple $\tuple{E, S}$ consisting
