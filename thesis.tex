...
% $Id: thesis.tex,v 1.83 1999-08-22 18:15:14 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\pagestyle{myheadings}\markboth{$ $Revision: 1.83 $ $}{$ $Revision: 1.83 $ $}
\pagenumbering{roman}
...
along the false branch of the \code{if}.  If the true branch is taken,
...
\fixme{Pruned form does not satisfy the given SSA criteria.}
...
variable in the original program.  Every path $X \pathplus E$
from a node $X$ containing a definition of a variable to the \code{END}
node must contain every node $Y$ which is a use of that variable.
[Boundary conditions.]
...
``extra'' \phisigfunction[or]{s} not required by conditions 1 and 2.
...
smallest number of \phisigfunction{s} such that the above
...
information.}
\fixme{As with pruned SSA form, pruned SSI does not satisfy the
...
\}\\
\end{samplecode}
\caption{Implicit bounds checks on Java array references.}\label{fig:induction}
\end{myfigure}
