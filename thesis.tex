...
% $Id: thesis.tex,v 1.88 1999-08-27 21:33:25 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\pagestyle{myheadings}\markboth{$ $Revision: 1.88 $ $}{$ $Revision: 1.88 $ $}
\pagenumbering{roman}
...
where at least one of $X$ and $Y$ is a definition.  So without loss of
generality assume one of $X$ and $Y$ is a definition of $v$.  But this
contradicts our choice of $Z$.
\end{description}
...
The above lemma justifies our algorithm's skipping over any SESE
region not containing a definition or use of $v$ when placing
\phisigfunction{s} for $v$.

\begin{lemma}\label{ssi_dom}
...
of the SSI form definition and lemma \ref{sese_child} that they are
sufficient.
\end{proof}

In practice, the conditions of lemma \ref{ssi_dom} are too expensive
...
\begin{theorem}
The algorithm in figure~\ref{alg:SSIplace} is conservatively correct.
\end{theorem}
\begin{proof}
Lemma \ref{sese_child} states that the child region exclusion of
...
algorithm and the conditions in \ref{ssi_dom} to produce exactly
