...
% $Id: thesis.tex,v 1.35 1999-06-13 04:43:41 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{document}
\pagestyle{myheadings}\markboth{$ $Revision: 1.35 $ $}{$ $Revision: 1.35 $ $}
...
\subsubsection{Placing $\phi$- and \sigfunction{s}}
...
the extra $\phi$- and \sigfunction{s}.  Figures \ref{fig:deaddata} and
\ref{fig:deadalg} presents an algorithm to identify unused code in
...
in fact it is usually much less.\footnote{If the number of instruction
operands and $\phi$- and \sigfunction{} arities is limited by a
...
Note that our algorithm for placing $\phi$- and \sigfunction{s} in
SSI form is
...
\emph{omit} $\phi$- or \sigfunction{s}.  Most SSA construction
algorithms, by contrast, are \emph{pessimistic}; they assume no
$\phi$- or \sigfunction{s} are needed and attempt to determine where
...
figure~\ref{fig:evil}.  Proving that all but two nodes require $\phi$-
and/or \sigfunction{s} for $a$ in this example seems to inherently
...
to show that any $\phi$- or \sigfunction{s} can be removed, and
