...
% $Id: thesis.tex,v 1.87 1999-08-27 19:27:08 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\pagestyle{myheadings}\markboth{$ $Revision: 1.87 $ $}{$ $Revision: 1.87 $ $}
\pagenumbering{roman}
...
\begin{lemma}
A \phifunction for a variable $v$ is needed at a node $N$ in an SESE $s$
iff there exists a definition, \phisigfunction[or]{} for $v$ at some
node $D$ in $s$ where:
\begin{enumerate}
\item $N$ is not also contained in a smaller SESE than $s$, although
$D$ may be,
\item $N$ is reachable from $D$, and
\item $D$ does not dominate $N$.
\end{enumerate}
\end{lemma}
...
\begin{theorem}
The algorithm in figure~\ref{fig:SSIalg} is conservatively correct.
\end{theorem}
\begin{proof}
