...
%%
	...
	public DELAYSLOT(InstrFactory inf, HCodeElement source) {
	...
	public CC(InstrFactory inf, HCodeElement source, String assem,
	          Temp[] dst, Temp[] src) {
...
MOVE(e1, CONST<i,p>(c)) %pred %( is13bit(c) )% %{
 emit(new Instr(instrFactory, ROOT, "\t mov "+c+", `d0\n",
		new Temp[] { e1 }, null));
}%
// other MOVEs
MOVE<i,p>(e1, e2) %{ /* catch-all */
 emit(new Instr(instrFactory, ROOT, "\t mov `s0, `d0\n",
		new Temp[] { e1 }, new Temp[] { e2 }));
}%
MOVE<l>(e1, e2) %{ /* long (pair of int) register move */
 emit(new Instr(instrFactory, ROOT, "\t mov `s0l, `d0l\n",
		new Temp[] { e1 }, new Temp[] { e2 }));
 emit(new Instr(instrFactory, ROOT, "\t mov `s0h, `d0h\n",
		new Temp[] { e1 }, new Temp[] { e2 }));
}%
MOVE<f>(e1, e2) %{ /* floating-point register move */
 emit(new Instr(instrFactory, ROOT, "\t fmovs `s0, `d0\n",
		new Temp[] { e1 }, new Temp[] { e2 }));
}%
MOVE<d>(e1, e2) %{ /* double (pair of fp) register move */
 emit(new Instr(instrFactory, ROOT, "\t fmovs `s0l, `d0l\n",
		new Temp[] { e1 }, new Temp[] { e2 }));
 emit(new Instr(instrFactory, ROOT, "\t fmovs `s0h, `d0h\n",
		new Temp[] { e1 }, new Temp[] { e2 }));
}%
...
JUMP(NAME(l)) %{
 emit(new Instr(instrFactory, ROOT, "\t b "+l+"\n", null, null)); /* target list?! */
 emit(new DELAYSLOT(instrFactory, ROOT));
}%
JUMP(BINOP(ADD, e1, e2)) %{
 emit(new Instr(instrFactory, ROOT, "\t jmpl `s0+`s1, %g0\n", null,
		new Temp[] { e1, e2 })); /* target list?! */
 emit(new DELAYSLOT(instrFactory, ROOT));
}%
JUMP(e1) %{
 emit(new Instr(instrFactory, ROOT, "\t jmpl `s0, %g0\n", null,
...
CJUMP(e, true_label, false_label) %{
 emit(new CC(instrFactory, ROOT, "\t cmp `s0, 0\n", null,
	     new Temp[] { e }));
 emit(new CC(instrFactory, ROOT, "\t bne "+true_label+"\n", null, null));/*target!?*/
 emit(new DELAYSLOT(instrFactory, ROOT));
 /* the next two lines can hopefully be left out. */
 emit(new CC(instrFactory, ROOT, "\t ba "+false_label+"\n", null, null));/*target?!*/
 emit(new DELAYSLOT(instrFactory, ROOT));
}%
...
// labels.
LABEL(l) %{
 emit(new InstrLABEL(instrFactory, ROOT, l.toString()+":\n", ((LABEL) ROOT).label));
}%
...
CONST<i,p>(c)=r %{
 if (is13bit(c))
   ...
   emit(new Instr(instrFactory, ROOT, "\t sethi %hi("+c+"), `d0\n", new Temp[]{r},null));
   emit(new Instr(instrFactory, ROOT, "\t or `s0, %lo("+c+"), `d0\n",
...
NAME(s)=r %{
 emit(new Instr(instrFactory, ROOT, "\t sethi %hi("+s+"), `d0\n",
		new Temp[]{r},null));
 emit(new Instr(instrFactory, ROOT, "\t or `s0, %lo("+s+"), `d0\n",
		new Temp[] { r }, new Temp[] { r }));
}%
/*TEMP(t)=r should be handled by c-g-g. */

BINOP<i,p>(op, CONST(c), e)=r %pred %( isCommutative(op) && is13bit(c) )% %{
 emit(new Instr(instrFactory, ROOT, "\t "+bop(op)+" `s0, "+c+", `d0\n",
...
BINOP<i,p>(op, e1, e2)=r  %pred %( isShift(op) || isCommutative(op) )% %{
 emit(new Instr(instrFactory, ROOT, "\t "+bop(op)+" `s0, `s1, `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
BINOP<i,p>(ADD, e1, UNOP(NEG, e2))=r /* subtraction */ %{
 emit(new Instr(instrFactory, ROOT, "\t sub `s0, `s1, `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
...
BINOP<f,d>(ADD, e1, e2)=r %{
 ...
 emit(new Instr(instrFactory, ROOT, "\t fadd"+s+" `s0, `s1, `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
BINOP<f,d>(ADD, e1, UNOP(NEG, e2))=r %{
 String s =(Type.isDoubleWord(((Tree)ROOT).getFactory(),((BINOP)ROOT).type()))?"d":"s";
 emit(new Instr(instrFactory, ROOT, "\t fsub"+s+" `s0, `s1, `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
BINOP<f,d>(MUL, e1, e2)=r %{
 String s =(Type.isDoubleWord(((Tree)ROOT).getFactory(),((BINOP)ROOT).type()))?"d":"s";
 emit(new Instr(instrFactory, ROOT, "\t fmul"+s+" `s0, `s1, `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
BINOP<f,d>(DIV, e1, e2)=r %{
 String s =(Type.isDoubleWord(((Tree)ROOT).getFactory(),((BINOP)ROOT).type()))?"d":"s";
 emit(new Instr(instrFactory, ROOT, "\t fdiv"+s+" `s0, `s1, `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
/* FIXME: finish floating-point rules */

UNOP<i,p>(NEG, e)=r %{
 emit(new Instr(instrFactory, ROOT, "\t sub %g0, `s0, `d0\n",
		new Temp[] { r }, new Temp[] { e }));
}%
UNOP<i,p>(_2B, e)=r %{ /* byte is 8-bit signed */
 emit(new Instr(instrFactory, ROOT, "\t sll `s0, 24, `d0\n",
		new Temp[] { r }, new Temp[] { e }));
 emit(new Instr(instrFactory, ROOT, "\t sra `s0, 24, `d0\n",
		new Temp[] { r } , new Temp[] { r }));
}%
UNOP<i,p>(_2S, e)=r %{ /* short is 16-bit signed */
 emit(new Instr(instrFactory, ROOT, "\t sll `s0, 16, `d0\n",
		new Temp[] { r }, new Temp[] { e }));
 emit(new Instr(instrFactory, ROOT, "\t sra `s0, 16, `d0\n",
		new Temp[] { r } , new Temp[] { r }));
}%
UNOP<i,p>(_2C, e)=r %{ /* character is 16-bit unsigned */
 emit(new Instr(instrFactory, ROOT, "\t sll `s0, 16, `d0\n",
		new Temp[] { r }, new Temp[] { e }));
 emit(new Instr(instrFactory, ROOT, "\t srl `s0, 16, `d0\n",
		new Temp[] { r } , new Temp[] { r }));
}%
UNOP<i,p>(_2L, e)=r %{ /* make 64-bit word. */
 emit(new Instr(instrFactory, ROOT, "\t mov `s0, `d0l\n",
		new Temp[] { r }, new Temp[] { e }));
 emit(new Instr(instrFactory, ROOT, "\t sra `s0, 31, `d0h\n",
		new Temp[] { r }, new Temp[] { e }));
}%
...
MEM(BINOP(PLUS, CONST(c), e1))=r %pred %( is13bit(c) )% %{
 emit(new Instr(instrFactory, ROOT, "\t ld"+suffix((Typed)ROOT.exp)+" [`s0+"+c+"], `d0\n",
...
MEM(BINOP(PLUS, e1, e2))=r %{
 emit(new Instr(instrFactory, ROOT, "\t ld"+suffix((Typed)ROOT.exp)+" [`s0+`s1], `d0\n",
		new Temp[] { r }, new Temp[] { e1, e2 }));
}%
MEM(CONST(c))=r %pred %( is13bit(c) )% %{
 emit(new Instr(instrFactory, ROOT, "\t ld ["+c+"], `d0\n",
		new Temp[] { r }, null));
}%
MEM(e)=r %{
 emit(new Instr(instrFactory, ROOT, "\t ld [`s0], `d0\n",
		new Temp[] { r }, new Temp[] { e }));
}%
