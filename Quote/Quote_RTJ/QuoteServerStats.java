...
public class QuoteServerStats { 
	...
	QuoteServerStats.SERVER_PORT=Integer.parseInt(args[0]);
	QuoteServerStats server = new QuoteServerStats(); 
	server.serveQuotes(); 
    } 
...
//  	int numSockets = 0;
	...
	try { 
//  	    while(keepRunning && (numSockets < 2450)) { 
	    while(keepRunning) {
		// Accept a new client. 
...
//  	    System.exit(1);
	...
	StockQuoteHandlerStats newHandler = new 
	    StockQuoteHandlerStats(clientSocket,stockInfo);
 ...
 * a specific client. 
 */ 
class StockQuoteHandlerStats implements Runnable { 
     ...
     */ 
    public StockQuoteHandlerStats(Socket newSocket, 
				  String[] info) { 
	mySocket = newSocket; 
	stockInfo = info; 
    } 
