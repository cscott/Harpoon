...
* @version 1.1 
*/ 
public class QuoteServerVTMemory { 
	...
	QuoteServerVTMemory.SERVER_PORT=Integer.parseInt(args[0]);
	QuoteServerVTMemory server = new QuoteServerVTMemory(); 
	server.serveQuotes(); 
    } 
	...
	StockQuoteHandlerVTMemory newHandler = new 
	    StockQuoteHandlerVTMemory(clientSocket,stockInfo);
	
	RealtimeThread newHandlerThread = 
	    new RealtimeThread(new VTMemory(100000, 100000), newHandler); 
 ...
 * a specific client. 
 */ 
class StockQuoteHandlerVTMemory implements Runnable { 
     ...
     */ 
    public StockQuoteHandlerVTMemory(Socket newSocket, 
				     String[] info) { 
	mySocket = newSocket; 
	stockInfo = info; 
    } 
