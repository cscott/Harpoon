...
class ImageTest
{
    ...
    protected static int[] edgex = {-1, 0, 1,
				    -2, 0, 2,
				    -1, 0, 1};
    protected static int[] edgey = {-1, -2, -1,
				    0,  0,  0,
				    1,  2,  1};
    protected static long start;
    protected static int width, height, rhcount=0;
    protected static int[] pix;

    public static void main(String[] args)
    {
	int T1 = Integer.valueOf(args[1]).intValue(); //definite edge threshold
	int T2 = Integer.valueOf(args[2]).intValue(); //possible edge threshold
	//int T3 = Integer.valueOf(args[3]).intValue(); //minimum accumulator value for Hough
	//int T4 = Integer.valueOf(args[4]).intValue(); //minimum number of hits for lines
	//int T5 = Integer.valueOf(args[5]).intValue(); //maximum misses in line tracing
	...
	try
	{
	    bimg = ImageIO.read(new File(args[0]));
	} catch (Exception e)
	{
	    System.out.println("IOException while reading file");
	}

	width = bimg.getWidth(c);
	height = bimg.getHeight(c);
	...
	int[] outvals1 = new int[width*height];
	int[] outvals2 = new int[width*height];
	int[] pixel = new int[3];
	int dx0, dx1, dx2, dy0, dy1, dy2, out;
	int kernindex, pixindex, outindex = 0, numpix;
	...
	ras1.setSamples(0,0,width,height,1,outvals);
	...
	System.out.println("Recursive hysteresis - " + rhcount + " calls - " + (System.currentTimeMillis()-start) + "ms");
	ras2.setSamples(0,0,width,height,1,pix);
	...
	ras3.setSamples(0,0,width,height,1,outvals);
	...
	for(r=0;r<2;r++)
	    for(t=0;t<PRECISION;t++)
		if(accumulator[r][t]>0) accum2[r][t]=255;
	for(r=PRECISION-2;r<PRECISION;r++)
	    for(t=0;t<PRECISION;t++)
		if(accumulator[r][t]>0) accum2[r][t]=255;
	for(t=0;t<2;t++)
	    for(r=0;r<PRECISION;r++)
		if(accumulator[r][t]>0) accum2[r][t]=255;
	for(t=PRECISION-2;t<PRECISION;t++)
	    for(r=0;r<PRECISION;r++)
	        if(accumulator[r][t]>0) accum2[r][t]=255;
		*/
	...
	ImagePanel panel = new ImagePanel(bimg, img1, img2, img3, img4, img5);
	frame.getContentPane().add(panel, BorderLayout.CENTER);
	frame.setSize(new Dimension(1440, 720));
...
class ImagePanel extends JPanel 
{
    ...
    public ImagePanel(Image image1, Image image2, Image image3, Image image4, Image image5, Image image6)
    {
        this.image1 = image1;
	this.image2 = image2;
	this.image3 = image3;
	...
	g.drawImage(image2, 480, 0, this);
	g.drawImage(image3, 960, 0, this);
	g.drawImage(image4, 0, 360, this);
	g.drawImage(image5, 480, 360, this);
	g.drawImage(image6, 960, 360, this);
    }
}
