...
% $Id: pldi02.tex,v 1.17 2001-11-28 19:35:31 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
non-zero mostly-$N$ fields.  The \code{202\_jess} benchmark contains
the field \code{Token.tag} which is always 0, 1, or 2.  Unfortunately
for us, its values are fairly evenly distributed in that range; we use
the zero value in our transformation.  The \code{213\_javac} benchmark
contains many instantiations of the \code{java.lang.Integer} class,
...
\code{228\_jack} contains values between 0 and 4 in the
\code{expectedModCount} field of \code{java.util.Hashtable}'s
...
\code{202\_jess}.  Our performance correlates closely with the
number of allocated object bytes in the benchmark; \code{201\_compress}
...
object-oriented benchmarks such as \code{202\_jess},
\code{213\_javac} and \code{228\_jack}, we excel.  Interestingly
...
optimize applications like \code{201\_compress}.  More sophisticated
...
signal-processing applications like \code{222\_mpegaudio} to
extensively object-oriented programs such as \code{202\_jess} and
\code{228\_jack}.
