 ...
 * @version $Id: DominatingMemoryAccess.java,v 1.3.2.1 2002-02-27 08:33:37 cananian Exp $
 */
   ...
   class Live {
      ...
      private void doOne(Stm root, Tree m) {
         ...
         if(m.kind() == TreeKind.MEM) {
            assert stmToMem.containsKey(root) == false : "******* Stm*" 
                        + harpoon.IR.Tree.Print.print(root) 
                        + "\n******* tree* " 
                        + harpoon.IR.Tree.Print.print(m);
            assert memToStm.containsKey(m) == false;
            stmToMem.put(root, m);
            memToStm.put(m, root);
         }
      }
      ...
      public HCodeElement use(HCodeElement hce) {
         if(stmToMem.containsKey(hce)) {
            ...
            if(useDefMap.containsKey(m)) {
               ...
               assert defUseMap.containsKey(dom);
               assert memToStm.containsKey(dom);
               return dom;
            }
         }
         return null;
         ...
         while(!interNodes.empty()) {
            ...
            for(Iterator rit = regs.iterator(); rit.hasNext(); ) {
               ...
               if(takenDA.contains(danum) == false) {
                  assert ref2dareg.containsKey(hce) == false;
