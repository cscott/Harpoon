...
COUNT_DECL
{
    ...
    if(hhdr -> hb_obj_kind == PREALLOC) return;
#endif
...
#ifdef WITH_PREALLOC_OPT
    /* Some memory ares cannot be deallocated (but are treated by the
       "mark" step as any NORMAL memory area.  Added for the
       preallocation optimization [AS] */
    if(obj_kind == PREALLOC) return;
#endif

    sz = hhdr -> hb_sz;
    ok = &GC_obj_kinds[obj_kind];
