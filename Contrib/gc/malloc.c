...
/* Added for preallocation optimization [AS]
   Allocate lb bytes of composite (pointerful) data in a memory
   area that is marked as any NORMAL area would be, but is not
   deallocated in the sweep step. */
# ifdef __STDC__
    GC_PTR GC_malloc_prealloc(size_t lb)
# else
    GC_PTR GC_malloc_prealloc(lb)
    size_t lb;
# endif
{
register ptr_t op;
...
#	else
	  lw = ALIGNED_WORDS(lb);
#       endif
	opp = &(GC_pobjfreelist[lw]);
	FASTLOCK();
        if( EXPECT(!FASTLOCK_SUCCEEDED() || (op = *opp) == 0, 0) ) {
            FASTUNLOCK();
            return(GENERAL_MALLOC((word)lb, PREALLOC));
        }
        /* See above comment on signals.	*/
	GC_ASSERT(0 == obj_link(op)
		  || (word)obj_link(op)
