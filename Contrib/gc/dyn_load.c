...
# if (defined(LINUX_THREADS) || defined (USER_THREADS) || defined(SOLARIS_THREADS) \
...
void GC_register_dynamic_libraries()
{
    ...
    /* Read /proc/self/maps	*/
        if (-1 == f) f = open("/proc/self/maps", O_RDONLY);
        if (-1 == f) ABORT("Couldn't open /proc/self/maps");
	if (0 != lseek(f, 0, SEEK_SET)) ABORT("/proc/self/maps seek failed");
