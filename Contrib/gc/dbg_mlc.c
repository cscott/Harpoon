...
/** Added for preallocation opt [AS] */
# ifdef __STDC__
    GC_PTR GC_debug_malloc_prealloc(size_t lb, GC_EXTRA_PARAMS)
# else
    GC_PTR GC_debug_malloc_prealloc(lb, s, i)
    size_t lb;
    char * s;
    int i;
#   ifdef GC_ADD_CALLER
...
# endif
{
    GC_PTR result = GC_malloc_prealloc(lb + DEBUG_BYTES);
    
    if (result == 0) {
        GC_err_printf1("GC_debug_malloc_prealloc(%ld) returning NIL (",
        	       (unsigned long) lb);
        GC_err_puts(s);
        GC_err_printf1(":%ld)\n", (unsigned long)i);
        return(0);
...
#    ifdef WITH_PREALLOC_OPT
      case PREALLOC:
	result = GC_debug_malloc_prealloc(lb, OPT_RA s, i);
 	break;
#    endif
#    ifdef ATOMIC_UNCOLLECTABLE
      case AUNCOLLECTABLE:
