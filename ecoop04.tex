...
% $Id: ecoop04.tex,v 1.5 2003-12-08 09:08:04 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[runningheads]{csallncs}
%\usepackage{times}
\usepackage{epsfig}
...
Cambridge, MA 02139 \\
\email{\{cananian,rinard\}@csail.mit.edu}
}
...
\begin{abstract}
  ...
  non-blocking software transactions.  We integrate transactions into
  Java to provide race-safety for certain types of improperly
  synchronized code, to provide fault-tolerance and backtracking, and
  to express synchronization and optimistic concurrency.  We use ideas
...
%\note{Also, transactions are exception-handling?}
  ...
  A model for the software transaction implementation is given in
  Promela, whose correctness has been mechanically verified using the
  \textsc{Spin} model checker.  We present a detailed design of our
  proposal, justifying the choices we make with empirical performance
  results and describing compiler optimizations which greatly improve
  the common-case efficiency for typical Java benchmarks.  We argue
  that our transaction mechanism makes some racy existing code safe,
  is easy to use for new code, and promises better scalability than
  the existing mutual-exclusion--based synchronization methods.
\end{abstract}
...
atomicity and fault-tolerance properties.  In this paper we will also
show how transactions can be used as a general exception-handling
mechanism and to implement back-tracking.

Although transactions can be implemented using mutual exclusion
(locks), our algorithms will utilize non-blocking synchronization to
exploit optimistic concurrency among transactions.
Non-blocking synchronization offers a number of advantages; foremost
for the concerns of this paper is fault-tolerance.
...
means for specifying thread-safety properties, and 
We will first examine destructive list traversal, showing how a
transaction implementation can be treated as an exception-handling
mechanism.   We then, using a network flow example, show how this
...
Finally, we show an existing race in the Java standard libraries (in 
...
We implement this with transactions, always initially attempting
...
We'll turn our attention now to parallel codes.
...
we have required the programmer to insert an \texttt{id} field into
...
From the programmer's point of view, we've also eliminated the
...
Further, we can implement {\tt atomic} using the {\tt try}/{\tt else}
exception-handling mechanism we've already introduced:
 \par {\footnotesize\samepage
\begin{verbatim}
while(true) {
  try {
...
We non-deterministically choose to execute the body of the {\tt
  atomic} block if and only if it will be observed by all to execute
atomically.  The same linguistic mechanism we introduced for
...
\vspace*{5mm}
We have designed a software transactions implementation which allows
us to efficiently implement the {\tt try}/{\tt else} and {\tt atomic}
constructs.  We have evaluated it by automatically
``transactifying'' Java applications by replacing synchronization with
{\tt atomic} blocks, and in \secref{properties} we present
experimental results describing the
transaction properties of programs so transformed.
We then present our software transaction design, based on the
properties we observed in our benchmarks.  We present modelling
results affirming the correctness of the design, and compiler
optimizations to enhance its performance.  We also describe a variant
on the scheme which uses some hardware support.  Finally, we present
...
correctness of the benchmarks we've examined.  Moreover, as
...
{\tt 201\_compress} & 2,981,777,890 & 2,272 & $<$0.1\% & 1,352 \\
{\tt 202\_jess} & 405,153,255 & 4,892,829 & 9.1\% & 4,826 \\
{\tt 205\_raytrace} & 420,005,763 & 4,177 & 1.7\% & 875,925 \\
{\tt 209\_db} & 848,082,597 & 45,222,742 & 23.0\% & 148,374 \\
{\tt 213\_javac} & 472,416,129 & 668 & 99.9\% & 8,928,356 \\
{\tt 222\_mpegaudio} & 282,263,132 & 1,675 & $<$0.1\% & 1,339 \\
\end{tabular}
\end{center}
...
\end{figure}
\note{mpegaudio number is for -s10 run; missing 228jack}
...
concurrency.  In order work \cite{AnanianAsKuLeLi04} we have shown
...
code.  Our basic mechanism here will be loosely based on the
...
\figref{tr-multi-obj} illustrates the basic data structures of the
