...
#ifdef RTJ_DEBUG
  printf("LTMemory.initNative(0x%08x, 0x%08x, %d, %d)\n", 
...
#ifdef RTJ_DEBUG
  current = mamb;
  printf("  MemBlock list for current LTMemory: ");
  while (current != NULL) {
...
void LTScope_freeAll(struct MemBlock* topmem) {
  struct MemBlock* current;
...
#ifdef RTJ_DEBUG
  checkException();
  printf("LTMemory.doneNative(0x%08x, 0x%08x)\n", env, memoryArea);
#endif
...
void* LTScope_RThread_MemBlock_alloc(struct MemBlock* mem, 
  ...
  void* ptr;
  checkException();
  printf("LTScope_RThread_MemBlock_alloc(0x%08x, %d)\n", mem, size);
...
#ifdef RTJ_DEBUG
  printf("LTScope_RThread_MemBlock_free(0x%08x)\n", mem);
#endif
...
#endif
    LTScope_freeAll((struct MemBlock*)(mem->block_info->allocator));
  }
}
...
void* LTScope_NoHeapRThread_MemBlock_alloc(struct MemBlock* mem, 
  ...
  void* ptr;
  checkException();
  printf("LTScope_NoHeapRThread_MemBlock_alloc(0x%08x, %d)\n", mem, size);
  printf("  current usage: %d of %d\n",
	 (size_t)((mem->block->free)-(mem->block->begin)),
	 (size_t)((mem->block->end)-(mem->block->begin)));
  printf("  begin: 0x%08x, free: 0x%08x, end: 0x%08x\n",
