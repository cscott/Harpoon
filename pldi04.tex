...
% $Id: pldi04.tex,v 1.22 2003-11-12 06:53:42 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
synchronize concurrent modifications to multiple objects.  A solution
using transactions to implement atomic regions is given, which greatly
...
version of the code can scale better and be faster than the locking version
...
Flanagan and Qadeer \cite{FlanaganQa03} demonstrated this with an
...
public final class StringBuffer ... {
  ...
  StringBuffer append(StringBuffer sb) {
    ...
A:  int len = sb.length(); // len may be stale.
...
The documentation indicates that the methods of this class are meant
...
Both the network flow example and this one require synchronization of
changes to more than one object.  Monitor synchronization is not
...
and provides a more intuitive specification for the desired
...
back-tracking.}
Schorr and Waite described a traversal mechanism...
\cite{SchorrWa67}
...
void traverse(List l) {
  ...
  for (int i=0; i<2; i++) {
    ...
    l = last;
    // now do again, backwards.
  }
}
\end{verbatim}
}

Now imagine that an exception occurs inside the {\tt visit()} method
at some point during the traversal.  Control leaves the {\tt
  traverse()}, but the data structure is left in shambles.

The {\tt atomic} block is one way to expose underlying transactions to
the programmer.  An alternative is to model transactions as an
...
This construct can also be implemented with transactions.  this view
of transactions as exception handling...  in fact, the {\tt atomic}
block we used in the previous two examples can be implemented using
{\tt try \ldots else}:
 \par {\footnotesize\samepage
\begin{verbatim}
while(true) {
...
If instead of an explicit exception, we make provision for transaction
failure due to transient system faults, we can see that the {\tt try
  else} block provides fault-tolerance, by allowing us to safely
re-execute portions of a program ...

This can be used for backtracking as well.
