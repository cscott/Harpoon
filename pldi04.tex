...
% $Id: pldi04.tex,v 1.9 2003-11-10 07:49:52 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Designing Efficient Transactions}
...
\section{EXTRA TEXT}
xxx x xx x xxx xxxx xxx xxx xx x x x x x xxxxxxxx xx x x x x
...
\secput{boundless}{The case for boundless transactions}

[The organization of this section needs to be rethought from scratch.]

This section illustrates how boundless transactional memory can ease
...
\par {\footnotesize\samepage  % Note:  Don't call the variable ``flow'', it makes the code not fit in the column. -Bradley
\begin{verbatim}
...
\note{CSA: I think this is where Charles wanted actual lock overhead
  numbers to be inserted.  Using FLEX, the locking code is over 11x
...
Transactional memory (and some language support) allows the
...
Several software transactions have been proposed.  Some constrain the
...
\secput{sw}{The case for HTM over STM}
