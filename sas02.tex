...
% $Id: sas02.tex,v 1.10 2002-05-11 10:59:50 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item {\bf Claz Compression:} Our rapid type analysis
...
\end{tabular}
\caption{(a) The {\tt JValue} class.
         (b) Portions of the {\tt java.lang.String} class.}
\label{fig:value}\label{fig:string}
\end{figure}
...
\centering (a) & \centering (b)
\end{tabular}
\caption{(a) Static specialization of {\tt java.lang.String}.
         (b) Dynamic selection among specialized classes in a method
             from {\tt java.lang.String}.
}
\label{fig:split}
\end{figure}

In practice, most strings are created as explicit substrings of other
strings, so
...
space savings. The combination of claz compression 
...
We abstract sets of non-singleton integer values into a tuple
...
\end{eqnarray*}%
\caption{Some combination rules for bit-width analysis.
...
Multiple writes to {\tt f} are permitted, unlike the Java-{\tt final}
fields.
...
subclasses can write to the field in objects of the ``big'' type
without restriction.
...
when space is truely at a premium.  Preliminary investigation
...
necessary to realize the identified possible gains.  In order to
...
Although this research was conducted using FLEX, which is a static
compiler, it would be straightforward to use \emph{extent analysis}
\cite{sreedhar00} to apply transformations to the closed-world
portions of a program which used dynamic class loading.  The space
allocated to the claz index could be updated during garbage collection
...
FLEX is a whole-program static compiler.
...
We obtain as much as 40\% reduction in live data on the {\tt
  javac} benchmark, with almost all of this coming from our
...
languages~\cite{ananian:siliconc,ananian99:tech,RR00:PLDI00,SBA00:PLDI00,BGSW00}.
