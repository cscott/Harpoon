...
% $Id: sas02.tex,v 1.4 2002-05-10 22:55:18 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
Figure~\ref{fig:value}a presents the {\tt NValue} class, which is 
...
value of the offset field.  Figure~\ref{fig:split}a illustrates
this transformation.
...
A join on two negative numbers yields the entry \code{(M\_\_)},
indicating the set of all negative non-zero numbers; a
join on a negative and a positive number yields \code{(M\_P)}, and so on.
\begin{figure}[tp]%
\begin{center}%
\parbox[b]{6cm}{\centering%
\renewcommand{\figscale}{0.4}\input{Figures/THlat1b}\\[.3cm]
(a)}%
\parbox[b]{6cm}{\centering%
\renewcommand{\color}[2][rgb]{}%ignore color commands
\renewcommand{\figscale}{0.4}\input{Figures/THlat6c}\\
(b)}%
\end{center}
