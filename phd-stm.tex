...
\subsection{Implementing the Java Native Interface}
...
\markboth{CHAPTER 5:~~APEX PERFORMANCE}{}
% lots more performance-related quotes at
% http://www.nonstopenglish.com/reading/quotations/k_Performance.asp
\epigraphhead[70]{\epigraph{%
All you owe the public is a good performance.
}{\textit{Humphrey Bogart}}}
...
sources were from the head CVS revision on 2007-05-27.  C sources were
compiled using \texttt{gcc-3.4}.  \appref{cmd}
...
benchmarks in \secref{full-bench}.  In this section I also freely
...
PowerPC hardware fully described in \charef{perf}.
...
199\%, as shown in \figref{phd-counter}.
...
aggressive optimization and scheduling.\footnote{This
  ...
  exceed the actual cost.  \punt{\Secref{full-bench} presents actual
...
The limitations noted in \secref{apex-limits}
...
\secref{auto}.
\figref{perfnums} lists the 7 benchmarks in SPECjvm98.  As in the
...
\epsbigfigput[Actual versus predicted nontransactional check
  overhead.]{phd-spec-nt-pred}{Actual (solid) versus predicted (striped)
...
microbenchmark of \secref{counter-bench}.  The \apex
...
\secref{counter-bench}, compiler limitations account for some of the
...
\figref{phd-spec-nt-pred} shows the performance predicted by \eqnref{nt-perf}
...
\figref{phd-spec-full} presents the performance of the transactional
versions of the SPECjvm98 benchmarks.  As in
\figref{phd-spec-nt-pred}, the leftmost bar represents the
...
\epsbigfigput[Transaction overhead for SPECjvm98.]{phd-spec-full}{%
  Transaction overhead for SPECjvm98.  The leftmost ``Base'' bar is compiled
  with method cloning and desugaring, all transactions removed, and
  direct reads and writes.  The ``No checks'' bar transforms methods, creates
  commit records at the start of each transaction, and checks for abort
  after every call, but uses direct loads and stores for all accesses.
  The ``Transactional (nonarray)'' bar is fully transactional for
  object accesses, but uses direct loads and stores for reads and
  writes of arrays.
  The ``Transactional'' bar is fully transactional.  The striped final bar
  shows predicted performance for the benchmark, broken down by the
  modeled source of the overhead.}

The second bar in \figref{phd-spec-full} shows
...
o_t = k_t t + k_{c_T} c_T
\end{equation}
where $o_t$ is the overhead fraction due to this portion of the
transaction transformation, $t$ is the transaction rate, $c_T$
is the number of method calls which are inside transactions, and $k_t$
and $k_{c_T}$ are unknown constants.  The least squares solution is:
\begin{eqnarray*}
k_t &=& 855 \text{ ns}/\text{transaction} \\
k_{c_T} &=& 18.2\text{ ns}/\text{method call within transaction}
\end{eqnarray*}
...
\figref{phd-spec-full} the red striped component
represents $o_t$, computed according to \eqnref{t-perf}.  The
prediction tracks the actual measured values (red solid bars) within
21\% of the actual overhead (ignoring \texttt{raytrace} and
\texttt{mpegaudio}, where $o_t$ is within the measurement
noise).

I can use \eqnref{nt-perf} from the previous section to compute the
overhead contribution from nontransactional read and write checks.
The light green and light blue striped portions of the predicted overhead in
\figref{phd-spec-full} represent $o_{NT}$, computed using
\eqnref{nt-perf} from the rate of nontransactional reads and writes
...
and coalescing (\secref{hoist}).

The model I use for transactional access costs distinguishes
\defn{virgin writes} (\defn{virgin reads}),  which are writes (reads)
to an object which has not previously been written to (read from) in
the current transaction.  These accesses involve transaction
bookkeeping which is not required for subsequent accesses.  I also
accumulate the object sizes involved in a virgin write, to model the
costs of creating a transactional clone.  The resulting model is:
\begin{equation}
o_T = k_{w_v} w_v + k_{ws} w_v s_v + k_{r_v} r_v + k_{r_T} r_T
\end{equation}
where $o_{T}$ is the overhead fraction due to transactional accesses,
$w_v$ is the (transactional) virgin write rate, $s_v$ is the average
size of an object targeted by a virgin write, $r_v$ is the
(transactional) virgin read rate, $r_T$ is the transactional read
rate, and $k_{w_v}$, $k_{ws}$, $k_{r_v}$, and $k_{r_T}$ are unknown
constants.  Solving for the constants yields:
\begin{eqnarray*}
 k_{w_v} &=& 680 \text{ ns}/\text{virgin write}\\
 k_{ws} &=& 85 \text{ ns}/\text{byte targeted by a virgin write}\\
 k_{r_v} &=& 660 \text{ ns}/\text{virgin read}\\
 k_{r_T} &=& 54 \text{ ns}/\text{transactional read}
\end{eqnarray*}
The $k_{w_v}$ (virgin write) and $k_{r_v}$ (virgin read) costs are
comparable to $k_t$, since the primary component of all of these costs
is memory allocation (of a clone object, a reader list entry, and a
transaction commit record, respectively).  When the object touched by
the virgin write is large, $k_{ws}$ accounts for the more expensive
allocation and initialization.  The transactional read cost, $k_{r_T}$
is comparable to $k_{w_{NT}}$, the nontransactional write cost.  The
operations involved are similar: a read of the head of the reader
list, a comparison, and then typically an access to the field
location.  The light green, green, light blue, and blue striped bars in
\figref{phd-spec-full} reflects these component of the cost prediction.
The overall prediction is within 10\% of actual costs for every
application except for \texttt{raytrace}, where the prediction
underestimates actual costs by 38\%.

The constants in our model were derived from the transactional
performance.  I can validate the model by deriving predictions for the object
and array components of the overhead separately.
The green bar in \figref{phd-spec-full} represents variants of the
benchmarks which replaced array reads and writes with
direct accesses.  The resulting overhead includes $o_t$, the ``no
checks'' overhead, as well as both transactional and
nontransactional overheads for nonarray objects.  This breakdown was
motivated by the observation that $s_v$, the average size of an object
touched by a virgin write, in some cases varies quite a lot between
array and nonarray objects, as \figref{virgin-size} shows.
...
\Charef{largeobj} will discuss this problem at
length and propose a solution based on functional array data
structures.

The light green and dark green bars in \figref{phd-spec-full} show our
predictions for the nonarray variants of the benchmarks, based on the
$w_v$, $s_v$, $r_T$, and $r_v$ values for nonarray accesses.  These
track the solid green bar indicating actual performance fairly well.
...
among the 256 possible byte values, which implies that about $\frac{1}{2}$\% of
...
Benchmark & $t$ & $c_T$ & $r_{NT}$ & $w_{NT}$ \\ \hline
...
\hline
\end{tabular}
\caption{Transaction, call, and nontransactional read and write rates
  for SPECjvm98 benchmarks.}
\label{fig:rates1}
\end{figure}
\begin{figure}\small
\begin{tabular}{l|rrrr|rr}
          &       &       &       &       & nonarray & array \\
Benchmark & $w_v$ & $s_v$ & $r_v$ & $r_T$ & $s_v$ & $s_v$ \\ \hline
...
\hline
\end{tabular}
\caption[Transaction performance model inputs for SPECjvm98 benchmarks.]
 {Transaction performance model inputs for SPECjvm98
  benchmarks. Each benchmark lists $w_v$, the virgin write rate,
  $s_v$, the average object size touched by a virgin write, $r_v$, the
  virgin read rate, and $r_T$, the transactional read rate.  The
  nonarray and array values of $s_v$ are broken out in the final
  two columns.}
\label{fig:virgin-size}
\end{figure}

Our predictive model of \apex performance uses 8 parameters, ordered here
by their contribution to overall overhead:
\begin{itemize}
\item $r_{NT}$, the number of nontransactional reads.  Each
  nontransactional read adds 2.4 ns overhead.
\item $c_T$, the number of method calls within transactions.  Each
  call adds 18.2 ns overhead.
\item $r_T$, the number of transactional reads.  Each transactional
  read adds 54 ns overhead.
\item $w_{NT}$, the number of nontransactional writes.  Each
  nontransactional write adds 63.5 ns overhead.
\item $w_v s_v$, the product of the virgin write rate with the average
  object size touched in a virgin write.  This contributes 85 ns
  overhead per byte in an object touched by a virgin write.
\item $r_v$, the number of virgin reads.  Each virgin read adds 660 ns
  overhead.
\item $w_v$, the number of virgin writes.  Each virgin write adds 680
  ns overhead.
\item $t$, the number of transactions.  Each transaction adds 855 ns
  overhead.
\end{itemize}

The costs associated with $t$, $w_v$, and $r_v$ might be reduced with
the use of a faster allocator.
...
small numbers of large transactions.  \Charef{htm}
presents an alternative transaction system which is better suited for large
numbers of small transactions.

The programmer can also reduce the number of virgin reads and writes
by structuring transactions to group fields involved in a transaction
inside a single object.   Combining smaller transactions will
also tend to reduce the number of expensive virgin reads and writes.
...
reads are twenty times cheaper than transactional reads.
...
Reducing the size of objects written by a transaction will
reduce the $w_v s_v$ term and improve performance.  In particular,
...
multiple times, increasing overhead.  \Charef{largeobj} describes a
modification to \apex which avoids this dependence on object size,
removing the $w_v s_v$ term from the performance model.
...
the \defn{functional array} datatype, which I first review.
...
\punt{
to address this problem.  I've chosen Tyng-Ruey Chuang's
...
added complexity.
Our ``large object protocol,'' which we'll call \lapex, uses a
lock-free variant of Chuang's algorithm.  I present this variant in
\secref{lf-fun-arr}. 
}
