...
sources were from the head CVS revision on 2007-05-23.
...
I examine the SPECjvm98 benchmark suite, the collection of
practical Java programs previously described and characterized in
\secref{auto}. 
...
in the place of its multi-threaded variant {\tt 227\_mtrt}.  The
SPECjvm98 benchmarks include an expert system, a simple database,
a Java compiler, an audio encoder, and a parser generator.
...
\epsbigfigput[Nontransactional check overhead for SPECjvm98.]{phd-spec-nt}{%
  ...
  were compiled with the transformations in
  \secref{synctrans} (method cloning, desugaring) and all
  synchronization was removed.  The left bar in each group performed
  direct field access.  The middle bar had all reads performed
  with load-linked, and all writes performed with a
  load-linked/store-conditional pair.  The right bar had all
  reads and writes transformed using the \texttt{readNT} and
  \texttt{writeNT} protocols.}
