...
signed immediate instruction field (eliminating the need to load it
into a register), and combined the flag checks in
...
\note{Make new graph which shows this computation.}
...
continues, ``VM implementors are encouraged to avoid splitting their
...
\end{verbatim}
\caption[Dropbox example illustrating the use of condition variables
  in Java]{Dropbox example illustrating the use of condition variables
...
commit and the wait.\footnote{Note in the dropbox example in
...
% LocalWords:  subsumption TransactionAbortException mutex
