...
\section{Experimentally-derived Performance Limits}
...
writing a constant, but the \texttt{volatile} declarations ensure that
...
fail a number of times\footnote{About 3,600 times in the 1 billion
  repetitions of the write.}
...
seems to indicate substantial scope for improvement.
For example, figure XXYA shows the emitted assembly for the write
check shown in \figref{counter-write}, along with a hand-coded
improvement.  \note{blah blah blah.}  This results in an improved
write-check overhead of XASDA. \note{One of the problems is
representing the value of the SC, which is stored in a condition code.
Note that the branches to \texttt{unusualWrite} are difficult to express.}

Further performance improvement is possible by combining the read
check and the write check and optimizing by hand.  Figure XYA shows an
...
signed immediate instruction field.  The branches to \texttt{unusualRead()} and
\texttt{unusualWrite()} are difficult to express in this hybrid of C
and assembly, but small stubs could be written to save registers and
perform a branch with the appropriate calling conventions.
The read-and-write check overhead is improved to XYZ with these
optimizations.

It may be worth looking for closely at the instruction schedule for
this optimized counter loop to understand the fundamental performance
limits. \note{Details of instruction scheduling here?}

\note{Conclusion.}
Patterson and Hennesey's measurements indicate that load and store
instructions comprise N\% and M\% of dynamic instruction counts in
real programs.  The N\% read overhead and M\% write overhead measured
in this microbenchmark thus translate into Y\% net non-transactional
overhead.
...
% precisec backend (exception handling, etc)

\section{Transforming Synchronization}
\subsection{Analyses} % (check/field oracles)
...
% LocalWords:  Promela microbenchmark
