...
(which are themselves divided into array and non-array types).
Our flag value was carefully chosen so as not to be a NaN for either
...
\begin{figure}\sis\fontsize{9}{10}\begin{alltt}
\textit{/* Example illustrating use of preprocessor specialization */}
/* (from readwrite.c) */
...
\end{alltt}
\caption[Specializing transaction primitives by field size and object type.]
{Specializing transaction primitives by field size and object type.
\figref{special2} contains the relevant definitions from
\texttt{preproc.h}.  Note the use of the macros \texttt{likely} and
\texttt{unlikely}, which are defined to communicate static branch
prediction information to the C compiler.}
\label{fig:special1}
\end{figure}
\begin{figure}\sis\fontsize{9}{10}\begin{alltt}
\textit{/* Preprocessor specialization mechanism */}
/* (from preproc.h) */
...
\end{alltt}
\caption{Snippet from \texttt{preproc.h} which defines the macros used
in \figref{special1}.}\label{fig:special2}
\end{figure}
Figures \ref{fig:special1} and \ref{fig:special2} demonstrate how
...
\section{Limitations}
   1. sub/multi word stuff \label{sec:multiword}
   2. object.wait \label{sec:object-wait}
   3. arrays?
...
\section{Compiler optimizations}\label{sec:moreopt}
   ...
   5. read-only objects (can initialilze/read directly)
