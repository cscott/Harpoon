...
{\tt StringBuffer} (\secref{stringbuffer}).  In a weakly atomic system
this unsynchronized code causes the {\tt atomic} {\tt
  StringBuffer.append()} method to
...
I need to add some additional information to each object to
...
The {\tt readNT} function performs a non-transactional read of field $f$ from
...
long as we always add readers to the head of the list.  We must also
...
benchmarks in the \secref{full-bench}.  We will also feel free to
...
aggressive optimization and scheduling.\footnote{Note that this
  ...
  exceed the actual cost.  \Secref{full-bench} will present actual
...
In \charef{stm} I described the design of an efficient
...
must be invoked (see \secref{readT} and \appref{verification}).  This
need only be done once for each object read in this transaction;
\secref{hoist} described how these checks are hoisted and combined to
...
analyses of this sort; \secref{moreopt} contains additional analyses
...
\Secref{multiword} will discuss some of the other challenges involved in
