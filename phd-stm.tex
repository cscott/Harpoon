...
compiler and the implementation techniques I used.
I conclude by listing some limitations of the
...
sources were from the head CVS revision on 2007-05-24.  C sources were
...
elapsed time of 7.06 seconds.  Our read (write) rate is thus $142M$
reads (writes) / second.  Read checks cost 1.0 ns/read, and write
checks cost 14.1 ns/write.  We can predict the overhead fraction $o$
for an arbitrary program with the linear equation
$o = 1.0 r + 14.1 w$ where $r$ ($w$) is the read (write) rate.
...
are the unknown coefficients.  The solution yields overhead contributions
$k_{r_{NT}} = 2.4 \text{ ns}/\text{read}$ and
$k_{w_{NT}} = 63.5 \text{ ns}/\text{write}$.  These values are
comparable to the
$1.0 \text{ ns}/\text{read}$ and $13.1 \text{ ns}/\text{write}$
overheads of the optimized checks we wrote for the counter
microbenchmark of \secref{counter-bench}.  The \flex
check implementation is slower than the hand-tuned assembly code.
Reasons include the presence of subword and multiword accesses in the
full benchmarks, and the difficulty of properly scheduling loads and
stores when they are hidden inside \texttt{asm} statements which are
opaque to the \texttt{gcc-3.4} backend compiler.
...
the difference.
The linear model only approximates this behavior.  The actual
...
\texttt{db} has loose dependencies that lower its overhead.  The model
matches actual performance within 14\% for these benchmarks.
...
\secref{counter-bench}, compiler limitations account for part of the
check overhead. It is hard to
...
versions of the SPECjvm98 benchmarks.  The leftmost bar represents the
original performance, as before.  The next two bars represent partial
transaction transforms, used to evaluate different components of the
overhead.  The penultimate bar shows the overhead of the full
transactified benchmark.  The final bar shows predicted overhead for
the application, broken down by contribution.
...
\epsbigfigput[Transaction overhead for SPECjvm98.]{phd-spec-full}{%
  ...
  The penultimate bar is fully transactional.  The striped final bar
  shows predicted performance for the benchmark, broken down by
  contributing factor.}
...
transactions is large.  The second bar in \figref{phd-spec-full} shows
...
writes.   Four changes to the application remain: transaction records
...
to overhead at this stage.  The number of transactional method calls
is a secondary factor.  The ``create transaction''
contribution should be modeled as:
\begin{equation}
o_t = k_t t + k_c c
\end{equation}
...
\end{equation}
I obtain
$k_t = 859 \text{ ns}/\text{transaction}$ and
$k_a = 4 \text{ ns}/\text{transactional memory access}$ as a minimum
norm solution.  The high
value for $k_t$ represents the high cost of memory allocation under
...
\figref{phd-spec-full} represents $o_{NT}$, computed using
...
\figref{phd-spec-full} reflects this component of the cost prediction.
The overall prediction is within 70\% of actual costs.

The green bar in \figref{phd-spec-full} represents another way of
...
direct accesses.  The resulting overhead includes $o_t$, the ``create
transaction'' overhead, as well as both transactional and
...
\punt{In the extreme case, the SPECjvm98 benchmark
...
Benchmark & $w_T s_T/1000$& $r_{NT}/1000$ & $r_T/1000$ & $w_{NT}$ & $w_T$ & $t$ \\ \hline
jess & 22,652 & 26,700 & 3,047 & 4,054,334 & 16,873 & 408,416\\
raytrace & 7,036 & 26,104 & 403 & 3,608,662 & 121,731 & 301\\
db & 14,534,339 & 18,187 & 5,378 & 987,484 & 361,431 & 1,328,518\\
javac & 12,360,808 & 15 & 29,805 & 7,983 & 8,863,336 & 55\\
mpegaudio & 246 & 212,850 & 3 & 38,663,921 & 739 & 287\\
jack & 75,195 & 12,695 & 8,016 & 5,902,722 & 1,638,242 & 1,815,263\\
\hline
\end{tabular}
\caption{Read, write, and transaction rates for SPECjvm98 benchmarks.}
\label{fig:rates}
\end{figure}

Our predictive model of \apex performance uses 6 parameters, ordered here
by their contribution to overall overhead:\footnote{I've distributed
  the $r_T$ and $w_T$ components of $a_T$ in \eqnref{t-perf} in this
  presentation.}
\begin{itemize}
...
\item $t$, the number of transactions.  Each transaction adds 859 ns overhead.
\end{itemize}
...
presents an alternative better suited for large numbers of small transactions.
