...
Our first analysis is encapsulated in a FLEX class named
\texttt{GlobalFieldOracle}.  This is a global analysis to determine
fields which can/cannot be read/written to from a
transactional/non-transactional context. \note{Think about this more.}

We also use a collection of \texttt{CheckOracle}s to remove unneeded
read and write checks. XXX.
We hoist each check to its immediate dominator iff that node is
postdominated by  the current node and that node is dominated by the
definition of the variable referenced in the check.

This ensures that all paths to the original location of the check must
pass through the new location, and that the object involved in the
check can not be undefined at the new check location.

The process is
repeated until no checks can be moved higher.

Then we eliminate any check which is dominated by an identical check.
...
% LocalWords:  Promela microbenchmark Hennessy PowerPC setjmp longjmp
