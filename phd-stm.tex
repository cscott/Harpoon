...
As previously discussed in \charef{strongatom},
...
``non-transactional'' code, while \defn{weakly atomic} transaction
...
\epsfigput[Check overhead as percentage of total dynamic instruction count]
  {phd-counter-instr}{%
...
overhead, as shown in \figref{phd-counter-instr}.  This should be
...
aggressive optimization and scheduling.\footnote{Note that this
  ...
  exceed the actual cost.  \Charef{full-bench} will present actual
...
achieved.  I will conclude by listing some limitations of the
...
iff they are known never to participate in transactions which might
conflict.
...
Now let us consider sub-word-sized writes.  Chapter 17.6 of the Java
...
a special type annotation on methods which may call \texttt{wait()},
so that users are not misled into thinking a block is atomic which may
invoke something which calls \texttt{wait()}.}  The fundamental
...
described in \charef{synctrans}, the transaction object is passed as
...
transformations can be applied selectively to only those methods which
...
Note that for code like the example given in \figref{dropbox} the
...
\epsfigput[Non-transactional check overhead for SPECjvm98.]{phd-spec-nt}{%
  Non-transactional check overhead for SPECjvm98.  Both benchmarks in
  each pair were compiled with the transformations in
  \charef{synctrans} (method cloning, desugaring) and all
...
{Number of false flags read/written in SPECjvm98 benchmarks.  The
...
\epsfigput[Transaction overhead for SPECjvm98.]{phd-spec-full}{%
  ...
  non-transactional read and write protocols for non-array objects.
...
desugaring (\charef{synctrans}) which is done for the transaction
transformation.

In \charef{counter-bench} we examined a microbenchmark to discover the
...
microbenchmark.   As discussed in \charef{synctrans}, it is hard to
...
adds register spills and moves here which are necessary only in
...
Transactions which mutate arrays also account for a large amount of
...
which could improve our performance on transactional code; they have
...
As described in \charef{synctrans}, the basic transaction
...
read/written which are not guaranteed to be read/written by the
...
application, monitor locks on objects which do not escape their thread
...
individual objects which do not escape their method, thread,
...
As we've seen in \charef{full-bench}, the copying cost can become
excessive.
...
Chuang's algorithm, which I will present in \charef{lf-fun-arr}.
...
a discussion of operational complexity in \charef{lf-fun-arr}.}
...
never mutated after they are created.   In \charef{lf-fun-arr} we will
...
functional arrays in \charef{lf-fun-arr}.
...
\caption[Data structures to support non-blocking multi-object
  ...
  of a transaction system to our concrete design in \figref{tr-multi-obj}.)}
\label{fig:multi-o}
\end{figure}
\begin{figure}
\sis\small%
...
Compare this to \figref{tr-multi-obj}; we've successfully recast our earlier
...
% LocalWords:  bytecode desugaring ensureReader desugar superclass NaN JDK iff
% LocalWords:  parameterized datatype Herlihy ensureReader
