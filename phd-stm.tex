...
\end{verbatim}
\caption{Declaring objects with version lists in Promela.}\label{fig:promdecl}
\end{figure}
...
frequent, however, so we provide more efficient implementations with
the same semantics.

We will present the operations one-by-one.
...
All possible races are confined to the copy-back procedure, which we
...
main body of the array.  We will present a lock-free version of
Chaung's algorithm later.  We will begin by
...
Our large object support will use \emph{functional arrays} as
...
$O(\lg n)$ worst-case access or update.\footnote{We will return to
...
may be referenced within a transaction is overly limiting.  We will
...
We extend the implementation to allow the fields of any number of
