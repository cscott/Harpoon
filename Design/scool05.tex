...
% $Id: scool05.tex,v 1.30 2007-03-29 19:26:35 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{abstract}
  ...
  has been mechanically verified using the \Spin model checker.
...
fault tolerance, exception handling, and backtracking.
...
Many recent lightweight transaction system designs have used
...
using cache-line--based abstractions to match the standard
cache/memory system,
have inspired similar word-based Software Transactional Memory (STM) systems
...
compiler can emit transactional code which is, after initial overhead,
equal in performance with unsynchronized/non-transactional code.  In
...
every object touched before it can be used in a transaction.  Moreover,
the cost of opening $R$ objects for reading and $W$ objects for
writing is $O(R(R+W))$; that is, potentially quadratic in the number
of objects involved.  The transaction system we will present
in \secref{algo} does not require foreknowledge of the behavior of a
transaction, and does not blow up on transactions that touch a large
number of objects.
...
{\tt atomic} blocks are not visible until the outer transaction commits.
...
data-cache read/write ratios \cite[pp. 105, 379]{HennessyPa96}.
...
some other value $B$, before we resume and clobber $B$ with our
...
than its own.  The {\tt ensureReader} routine in \figref{ensurereader}
performs these two operations.
With sufficiently precise pointer analysis these steps
can be combined and hoisted so that they are done once before the
first read from an object and not repeated at every read.  Imprecision in the
...
\end{verbatim}
\caption{The per-object version-setup routine for transactional reads.}
\label{fig:ensurereader}
\end{figure}

Then, to read a field within a transaction,
the thread initially does the read from the original object.  If
...
\end{figure}
\vspace{.1cm}% put this in the next column; keep section head together
