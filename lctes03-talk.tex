% CVSId: $Id: lctes03-talk.tex,v 1.9 2003-05-23 23:50:53 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TO DO:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  - a slide to explain loops/our choice opposed to interval analysis
%  - cut 'push hard on all the bits' slide; replace by something
%    mentioning the magic words ``analysis'' ``transformation'' or
%    such.
...
%  - Flag, in words or a slide, the transition from analysis to use
%    between ``all cars are black'' and ``field compression using
%    bitwidths''
%  - Prettify the ``field packing'' slide.
%  - Don't miss ``BigString!''
%  - Reword ``Fields and Maps''; we don't need ``bottom'' and don't
%    mention it.  Can be abbreviated!  Just talk about ``default''
%    value which we don't have to store. ``Not an incomplete
%    mapping''.
%  - Before graphs & after ``how well does it work'' add an
%    ``experimental setup'' slide: MIT Flex compiler, x86, etc, etc.
%  - Mention ``lower is better'' and ``black is left over'' when
%    discussing results slides.
%  - Mention ``add on'' in ``moderate performance impact'' slide.
%  - Separate slide saying why memory opt important/memory wall.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[%
pdf,
colorBG,
...
With the new lattice, we start at $\bot$ as before. @ Looking at {\tt
  i=1}, @ we do a meet with $1$ @ to get $1$.  But now, when we see
{\tt i=2}, @ and meet with $2$, @ we get {\tt (\_\_P)}, or ``a positive
...
@ Looking at {\tt i=1}, @ we do a meet with $1$ @ to get $1$.
@ We look at {\tt i=2}, @ and meet with $2$, @
...
%---------------------------------------------------------------------- SLIDE -
% closer slide to avoid revealing the 'graveyard' inadvertently.
\begin{slide}{}
\vspace{.5cm}
