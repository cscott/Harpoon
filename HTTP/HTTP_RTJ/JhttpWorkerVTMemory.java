...
public class JhttpWorkerVTMemory extends Thread{
    private Socket client;
    public  int    fileLength, returnCode;
    private boolean logging;

    public JhttpWorkerVTMemory(Socket client, boolean logging) {
	this.client = client;
	this.logging=logging;
    }
    
    private class ParamsVTMemory {
    ...
    public void run() { 
	ParamsVTMemory params = new ParamsVTMemory();

	try {
	    HTTPResponseVTMemory resp = new HTTPResponseVTMemory();
		...
		case 0:
		    HTTPServicesVTMemory.GET_handler(params.fileName, 
						     out, resp);
		    break;
		case 1:
		    HTTPServicesVTMemory.HEAD_handler(params.fileName, 
						      out, resp);
		break;
		case 2:
		    HTTPServicesVTMemory.POST_handler(params.fileName, 
						      out, resp);
		    break;
		default:
		...
		try {
		    out.flush();
		    if (logging)
			LogFileVTMemory.write_log(client,
						  params.methodType, 
						  params.fileName,
						  params.httpVersion,
						  resp.returnCode, 
...
//*****************************************************************************
  ...
  private int method(BufferedReader in, ParamsVTMemory params){
      int ret = -1;
