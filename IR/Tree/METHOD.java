 ...
 * @version $Id: METHOD.java,v 1.3.2.1 2002-02-27 08:36:47 cananian Exp $
 */
public class METHOD extends Stm {
	...
	assert method!=null;
	assert retType==-1 || Type.isValid(retType);
	assert params!=null; assert(params.length>0);
	for (int i=0; i<params.length; i++) assert params[i].tf == tf;
	this.method = method;
	...
	assert paramsLength == params.length : "Can't change number of parameters to METHOD";
	...
	assert kids==null;
	assert tf==this.tf : "cloning Params not yet implemented";
