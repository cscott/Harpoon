 ...
 * @version $Id: MEM.java,v 1.3.2.1 2002-02-27 08:36:47 cananian Exp $
 */
public class MEM extends Exp implements PreciselyTyped {
	...
	assert Type.isValid(type) && exp!=null;
	...
	assert tf == exp.tf : "This and Exp must have same tree factory";
	assert (0<=bitwidth)&&(bitwidth<32) : "Invalid bitwidth";
    }
	...
	assert kids.tail==null;
	return new MEM(tf, this, type, kids.head, isSmall, bitwidth, signed);
    }

    // Typed interface:
    public int type() { assert Type.isValid(type); return type; }
    ...
    public int bitwidth() { assert type==INT&&isSmall; return bitwidth; }
    /** Returns true if this is a signed expression, false otherwise.
     *  Only valid if the <code>isSmall()==true</code>. */
    public boolean signed() { assert type==INT&&isSmall; return signed; }
