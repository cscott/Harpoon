...
% $Id: quads.tex,v 1.1 1998-09-08 14:31:21 cananian Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
representation (henceforth, ``IR'') is known as ``QuadSSA,'' referring
to its derivation from quadruple-style IRs and its SSA form.

\subsection{Quadruples}
Unlike expression-tree structures intermediate representations, where
...
statement, however, we have attempted to retain the atomic simplicity
of the quadruple form.

\subsection{Static Single-Assignment Form}

Quoting from \cite{appel:modern}:
\begin{quote}
...
obvious from the example is that unrelated uses of the same variable
...
SSA form becomes more complex when we move from straight-line code to
...
constraints, but leads to executable code.
...
You will find that analysis, transformation, and optimization of the
...
removed those checks with are redundant, impossible, or otherwise
...
statement for the exception return value.
...
get object ID numbers and source file information for debugging which
...
\tt HEADER & Dummy start node for control-flow graph.\\
\tt METHODHEADER & Same as \texttt{HEADER}, but with additional 
                   method-argument information.\\
\tt FOOTER & Dummy end node for control-flow graph.\\ \hline
