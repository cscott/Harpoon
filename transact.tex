...
% $Id: transact.tex,v 1.11 2000-09-28 22:57:07 cananian Exp $
...
\IF{$o\ptr\code{field}[n] = \code{FLAG\_VALUE}$}
  \STATE $v \gets o \ptr \code{hashunion.inflated} \ptr \code{first\_version}$
  \WHILE{$\text{StateP}(v \ptr \code{transid}) \neq \code{COMMITTED}$}
    \STATE $\text{AbortTransaction}(v \ptr \code{transid})$
% defer this work until next read:
%    \STATE \COMMENT{XXX: potential race here --- v could be already removed}
%    \STATE remove $v$ from version list atomically
%    \STATE $v \gets v\text{'s replacement}$
    \STATE $v \gets v\ptr\code{next}$ \COMMENT{watch for races}
  \ENDWHILE
...
\ENDLOOP*
\end{myalgorithmic}
\caption{Algorithm for determining transaction status.}
\label{fig:statep}
\end{figure}
