...
% $Id: transact.tex,v 1.19 2000-09-29 21:50:46 cananian Exp $
...
\STATE $v \gets o \ptr \code{hashunion.inflated} \ptr \code{first\_version}$
\STATE $r \gets \code{\&}(v\ptr\code{readers})$
\IF{$r\ptr\code{transid} \neq c$}\label{line:rtL1}
  \STATE $r \gets r\ptr\code{next}$
  \IF{$r \neq \code{null}$}
    \GOTO line \ref{line:rtL1}
  \ENDIF
  \IF{$c$ is not a subtransaction of $v\ptr\code{transid}$
      or $\text{StateP}(c) = \code{ABORTED}$}
    \STATE commit suicide \COMMENT{can't guarantee consistency}
  \ENDIF
  \STATE add $c$ to head of readers list, atomically.
\ENDIF
...
\begin{myalgorithmic}
\STATE $v \gets o \ptr \code{hashunion.inflated} \ptr \code{first\_version}$
\IF{$v\ptr\code{transid} \neq c$}
  \IF{$c$ is not a subtransaction of $v\ptr\code{transid}$
      or $\text{StateP}(c) = \code{ABORTED}$}
    \STATE commit suicide \COMMENT{can't guarantee consistency}
  \ENDIF
  \STATE kill some of the readers
...
\begin{figure}
$\text{FindVersion}(\code{struct oobj *}o, \code{struct commitrec *}c_w,
                    \code{boolean }wr)$~=
\begin{myalgorithmic}
\STATE $\code{InflateObject}(o)$
\STATE $v \gets o \ptr \code{hashunion.inflated} \ptr \code{first\_version}$
\IF{$v = \code{null}$}
  \RETURN $\text{CreateNewVersion}(o, c_w, o)$
\ENDIF
\STATE \label{line:L1}
       $c_c \gets v\ptr\code{transid}$
\IF{$c_c \neq c_w$}
  \IF{$c_c \neq \code{null} \wedge c_c\ptr\code{state} = \code{COMMITTED}$}
    \REPEAT
      \STATE $c_c \gets c_c\ptr\code{parent}$
    \UNTIL{$c_c = \code{null} \vee c_c\ptr\code{state} \neq \code{COMMITTED}$}
    \STATE $v\ptr\code{transid} \gets c_c$ \COMMENT{atomic}
  \ENDIF
  \IF[committed all the way down]{$c_c = \code{null}$}
    \STATE $v\ptr\code{next} \gets \code{null}$ \COMMENT{prune}
    \STATE $v \gets \text{CreateNewVersion}(o, c_w, v)$
  \ELSIF{$c_c\ptr\code{state} = \code{ABORTED}$}
    \STATE \COMMENT{XXX: potential race here? WAITING could become COMMITTED}
    \STATE \COMMENT{XXX: potential race here --- v could be already removed}
    \STATE remove $v$ from version list atomically
    \STATE $v \gets v\text{'s replacement}$
    \GOTO line \ref{line:L1}
  \ELSE
    \STATE $c_w' \gets c_w$
    \WHILE{$c_w' \neq c_c$}
      \STATE $c_w' \gets c_w'\ptr\code{parent}$
      \IF{$c_w' = \code{null}$}
        \IF{$wr$}
          \STATE commit suicide
        \ELSE
  ...
  \ENDIF
\ENDIF
\STATE \COMMENT{XXX: because of multiple readers, there could be
readers left to the right of this writer at this point.}
\RETURN $v$
\end{myalgorithmic}
\caption{Algorithm for matching \code{vinfo}'s with \code{commitrec}'s.}
\label{fig:findversion}
\end{figure}
