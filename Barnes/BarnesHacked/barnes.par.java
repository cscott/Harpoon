...
class vec { 
  ...
  synchronized void crossProd(vec u, vec w) { 
    v1 = u.v2*w.v3 - u.v3*w.v2;
    v2 = u.v3*w.v1 - u.v1*w.v3;
    v3 = u.v1*w.v2 - u.v2*w.v1;
  }
...
class NBodySystem {
  ...
  void pickshell(vec v, double rad){
    ...
    do {
      v.v1 = random.xrand(-1.0, 1.0);
      v.v2 = random.xrand(-1.0, 1.0);
      v.v3 = random.xrand(-1.0, 1.0);
      rsq = v.dotProd(v);
    } while (rsq > 1.0);
  ...
  void intcoord(int xp[], vec rp) {
    ...
    xsc = (rp.v1 - v.v1) / size;
    xp[0] = (int)(Math.floor(node.IMAX * xsc));

    xsc = (rp.v2 - v.v2) / size;
    xp[1] = (int)(Math.floor(node.IMAX * xsc));

    xsc = (rp.v3 - v.v3) / size;
    xp[2] = (int)(Math.floor(node.IMAX * xsc));
...
// ----------------------------------------------------------------------------
  ...
  void loadTestData(){
    ...
    for(k = halfnbody; k < nbody; k++) {
      ...
      tmpv2.v1 = tmpv.v1 + offset;
      cmr.add(tmpv2);

      tmpv2.v2 = tmpv.v2 + offset;
      cmr.add(tmpv2);

      tmpv2.v3 = tmpv.v3 + offset;
      cmr.add(tmpv2);
  ...
  void setbound() {
    ...
    tmp1.sub(tmp2);
    if(side < tmp1.v1) side = tmp1.v1;
    if(side < tmp1.v2) side = tmp1.v2;
    if(side < tmp1.v3) side = tmp1.v3;
