 ...
 * @version $Id: MethodMutator.java,v 1.5 2002-09-01 07:46:31 cananian Exp $
 */
public abstract class MethodMutator<HCE extends HCodeElement>
    ...
    private final HCodeFactory hcf = new SerializableCodeFactory() {
        private final Map<HMethod,HCode<HCE>> cache =
	    new HashMap<HMethod,HCode<HCE>>();
        public String getCodeName() {
	    return mutateCodeName(parent.getCodeName());
	}
        public HCode convert(HMethod m) {
            if (cache.containsKey(m)) return cache.get(m);
            HCode<HCE> hc = parent.convert(m);
            if (hc!=null)
		try {
