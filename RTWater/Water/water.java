...
// ----------------------------------------------------------------------
  ...
  void potengOuterSplit(int first, int num) {
    ...
    for (int i = 0; i < num; i++) { 
      t[i] = new potengThread(this, first+i);
      t[i].start();
    }
    for (int i = 0; i < num; i++) { 
      try { 
...
void interf2(skratch_pad p1, skratch_pad p2, vec Res1[]){
  double incr; 
  vec Res2[] = new vec[3];
  Res2[0] = new vec();
  Res2[1] = new vec();
...
void storeData(int dest){
  ...
  void interfInnerLoop(int idx, vec Res1[]){
    int i;
    skratch_pad p1, p2;
  ...
  void interfOuterLoop(){
    ...
    for(i = 0; i < numMol-1; i++) {
      ...
      Res1[1] = new vec();
      Res1[2] = new vec();
      interfInnerLoop(i,Res1);
      getPad(i).update_forces(Res1);
    }
  }

  void interfOuterSplit(final int first, final int num) {
    // Put this entire computation into its own region
    // Wes Beebee
    class compute implements Runnable {
      public void run() {
	interfThread t[] = new interfThread[num];
	for (int i = 0; i < num; i++) {
	  t[i] = new interfThread(ensemble.this, first+i);
	  t[i].start();
	}
	for (int i = 0; i < num; i++) {
	  try {
	    t[i].join();
	    getPad(first+i).update_forces(t[i].Res1);
	  } catch (java.lang.InterruptedException e) {
    ...
    CTMemory ct = new CTMemory(10000000);
    ct.enter(c);
  }
...
class potengThread extends Thread {
  ...
  potengThread(ensemble e, int i) {
    en = e;
    idx = i;
  }
  public void run() {
    en.potengInnerLoop(idx);
  }
}
