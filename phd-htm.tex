...
\paragraph{Understanding hardware modifications:}
...
LTM has the same semantics as those described in \secref{utm}.  The
...
%% \subheading{Instruction set interface and semantics}
%% 
...
\subheading{Cache modifications}
...
\subheading{Processor modifications}
...
\label{sec:javac}
Our initial results from \secref{benchmarks} suggested that since
...
%% \subheading{The \texttt{LinkedList} microbenchmark}
%% 
...
\subheading{Rolling back processor state}
...
% KA: This footnote is wrong: In section 5.2 of the paper they
% describe the same register checkpoint mechanism that we use.
% \footnote{In contrast, Rajwar and
% Goodman's speculative lock-elision scheme \cite{RajwarGo01} uses the
% branch-speculation hardware with no changes and uses (bounded)
% transactions to implement critical sections only when the instructions
% of the critical section fit within the reordering window of the
% processor.  We discuss speculative lock elision further in
% \secref{related}.}

%%% INLINE FIGURES 
% \figput{snapshot}%
% {UTM processor modifications. The S bit vector tracks the active
% physical registers.  For each rename table snapshot, there is an
% associated S bit vector snapshot.  The Register Reserved List holds
% the otherwise free physical registers until the transaction commits.
% The LPR field is the next physical register to free (the last physical
% register referenced by the destination architectural register).}

\subheading{Memory state}
...
\caption{UTM processor modifications. The S bit vector
...
\caption{The xstate data structure.
